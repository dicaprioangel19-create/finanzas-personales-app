...
function obtenerDatosSaldoAcumulado(lista) {
  ...
  // 👇 punto inicial inteligente en 0
  puntos.push({
...
function dibujarGraficoSaldoAcumulado(datos) {
  ...
  // 👇 si nunca bajó de 0, el eje inferior queda en 0
...
function dibujarTramosSaldo(datos, medidas) {
  ...
  for (let i = 0; i < datos.length - 1; i++) {
    ...
    let colorTramo = COLOR_SALDO_SUBE;

    if (siguiente.saldo < actual.saldo) {
      colorTramo = COLOR_SALDO_BAJA;
    } else if (siguiente.saldo > actual.saldo) {
      colorTramo = "#4fb37c"; // verde elegante al subir
    }
...
function mostrarTooltipGrafico(dato, indice, datos) {
  ...
  chartTooltip.style.left = `${Math.max(12, Math.min(leftVisual - 55, rect.width - 140))}px`;
