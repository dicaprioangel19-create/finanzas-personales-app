import { iniciarNotificaciones } from "./services/push-notifications.js";
import {
  STORAGE_KEY,
  BUDGET_STORAGE_KEY,
  PIN_STORAGE_KEY,
...
import {
  ...
  pinGuardado,
  modoLock,
  sincronizandoScrollGrafico,
  prorrateoCalendarDate,

  setMovimientos,
  setPresupuestos,
  setProrrateos,
  setEditandoId,
  setEditandoProrrateoId,
  setPinGuardado,
  setModoLock,
  setSincronizandoScrollGrafico,
  setProrrateoCalendarDate
} from "./core/state.js";
...
function configurarScrollGrafico() {
  ...
  lineCanvasScroll.addEventListener("scroll", () => {
    ...
    setsincronizandoScrollGrafico(true);
    chartRangeScroll.scrollLeft = lineCanvasScroll.scrollLeft;
    requestAnimationFrame(() => {
      setsincronizandoScrollGrafico(false);
    });
  });
...
setMovimientos(
  movimientos.filter((mov) => mov.id !== id)
);
...
setProrrateos(
  prorrateos.map((item) =>
    item.id === editandoProrrateoId
      ? prorrateoActualizado
      : item
  )
);
...
setProrrateos(
  prorrateos.filter((item) => item.id !== id)
);
